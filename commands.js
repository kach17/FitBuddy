...
const PREFERENCE_CONFIGS = {
  run: [
    { id: "walk", label: "Brisk Walking" },
    { id: "social", label: "Light Jogging" },
    { id: "push", label: "Distance Running" },
  ],
  hike: [
    { id: "nature", label: "Gentle Nature Walks" },
    { id: "trails", label: "Trail Hiking" },
    { id: "summit", label: "Summiting" },
  ],
  cycle: [
    { id: "casual", label: "Leisurely Rides" },
    { id: "group", label: "Steady Group Rides" },
    { id: "speed", label: "Long Distance Rides" },
  ],
};
...
function getMemberActivities(member) {
  ...
  return roles
    .filter((r) => member?.roles?.cache?.some((cr) => cr.name === r.role))
...
function buildReviewBeaconMessage(session, sessionId, member) {
  ...
  const components = [
    new ActionRowBuilder().addComponents(
      new ButtonBuilder()
        .setCustomId(`confirm-${sessionId}`)
        .setLabel("Post Beacon")
        .setStyle(ButtonStyle.Success),
      new ButtonBuilder()
        .setCustomId(`time-${sessionId}`)
        .setLabel("Edit Time")
        .setStyle(ButtonStyle.Secondary),
      new ButtonBuilder()
        .setCustomId(`city-${sessionId}`)
        .setLabel("Edit City")
        .setStyle(ButtonStyle.Secondary),
    ),
  ];
...
async function updateRegistryMessage(thread, eventId, hostId) {
  ...
  const registryMsg = pinnedMsgs.find(
    (m) => m.embeds[0]?.title === "Event Roster",
  );
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(desc)
    .setFooter({
      text: "Use the button below to drop out if your plans change.",
    });
  ...
  data: new SlashCommandBuilder()
    ...
    .addStringOption((opt) =>
      opt
        .setName("details")
        .setDescription(
          "Natural description (e.g., tomorrow 9am run at city park)",
        )
        .setRequired(true),
    ),

  async execute(interaction) {
    const parsed = parseBeaconInput(
      interaction.options.getString("details"),
      db.getUserDefaults(interaction.user.id),
    );
    if (!parsed.timestamp)
      return interaction.reply({
        content:
          "I couldn't quite figure out that time. Try something like 'tomorrow 9am'.",
        flags: 64,
      });

    const sessionId = `${interaction.user.id}_${Date.now()}`;
    db.savePendingSession(
      sessionId,
      interaction.user.id,
      parsed.activity,
    ...
    await interaction.reply({
      ...buildReviewBeaconMessage(parsed, sessionId, interaction.member),
      flags: 64,
    });
  },
};
...
export const profileCommand = {
  ...
  async execute(interaction) {
    ...
    const memberRoles = targetMember
      ? activityRoles.filter((r) =>
          targetMember.roles.cache.some((cr) => cr.name === r),
        )
      : [];
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        {
          name: "Home City",
          value: defaults?.default_city || "Not Set",
          inline: true,
        },
        {
          name: "Focus",
          value: defaults?.last_activity_type
            ? defaults.last_activity_type.charAt(0).toUpperCase() +
              defaults.last_activity_type.slice(1)
            : "Not Set",
          inline: true,
        },
      );
    ...
    if (memberRoles.length > 0) {
      embed.addFields({
        name: "Sports",
        value: memberRoles.map((r) => `${roleEmojiMap[r]}  ${r}`).join("\n"),
        inline: false,
      });
    } else {
    ...
    if (Object.keys(existingPrefs).length > 0) {
      embed.addFields({
        name: "Activity Styles",
        value: Object.entries(existingPrefs)
          .map(
...
export async function handleButtonInteraction(interaction, client) {
  ...
  if (interaction.isStringSelectMenu()) {
    if (action === "activity") {
      ...
      if (!session)
        return interaction.reply({ content: "Session timed out.", flags: 64 });
      return interaction.update(
        buildReviewBeaconMessage(session, targetId, interaction.member),
      );
    } else if (action === "preference") {
      const config = PREFERENCE_CONFIGS[targetId]?.find(
        (t) => t.id === interaction.values[0],
      );
      if (config) {
        db.saveUserPreference(interaction.user.id, targetId, config.label);
        await interaction.reply({
          content: `Got it. Your ${targetId} preference is set to ${config.label}.`,
          flags: 64,
        });
      }
      return;
  ...
  if (action === "join") {
    ...
    if (!db.logAttendance(eventId, interaction.user.id))
    ...
    const fieldIdx = embed.data.fields?.findIndex((f) =>
      f.name.includes("Participants"),
    );
    if (fieldIdx !== -1)
      embed.data.fields[fieldIdx].value =
        `${db.getEventAttendanceCount(eventId)} joined`;
    await interaction.editReply({ embeds: [embed] });
    return interaction.followUp({
      content: "You're in. Say hi in the thread!",
      flags: 64,
    });
  }
  ...
  if (action === "leave") {
    ...
    if (!event)
      return interaction.editReply({ content: "Event ended or not found." });
    if (event.host_id === interaction.user.id)
      return interaction.editReply({
        content: "Hosts cannot leave. Cancel event in thread.",
      });
    if (!db.removeAttendance(eventId, interaction.user.id))
      return interaction.editReply({ content: "You aren't on the list." });
        ...
        .addComponents(
          new ActionRowBuilder().addComponents(
            new TextInputBuilder()
              .setCustomId(isTime ? "newTime" : "newCity")
...
export async function handleModalSubmit(interaction) {
  ...
  if (type === "time") {
    const timestamp = parseBeaconInput(
      interaction.fields.getTextInputValue("newTime"),
      null,
    ).timestamp;
    if (timestamp)
      db.updatePendingSession(sessionId, {
        timestamp: timestamp.toISOString(),
      });
    else
      return interaction.reply({ content: "Unknown time format.", flags: 64 });
  } else if (type === "city") {
    db.updatePendingSession(sessionId, {
      city: interaction.fields.getTextInputValue("newCity"),
    });
  }

  return interaction.update(
    buildReviewBeaconMessage(
      db.getPendingSession(sessionId),
      sessionId,
      interaction.member,
    ),
  );
...
async function publishBeacon(interaction, client, eventData) {
  ...
  try {
    ...
    console.error(
      "Could not fetch dashboard channel, falling back to interaction channel.",
      error.message,
    );
  }
  ...
  const embed = new EmbedBuilder()
    .setColor("#5865F2")
    .setAuthor({
      name: `${interaction.user.username} is hosting a`,
      iconURL: interaction.user.displayAvatarURL(),
    })
    .setTitle(
      `${ACTIVITY_EMOJIS[eventData.activity] || "🎯"}  ${eventData.activity.charAt(0).toUpperCase() + eventData.activity.slice(1)} Session`,
    )
    .setDescription(
      `📅 **${formatEventTime(timestamp)}**  —  <t:${Math.floor(timestamp.getTime() / 1000)}:R>\n\n📍 **${eventData.location}**, ${eventData.city}\n${getMockWeather(eventData.city)}`,
    )
  ...
  const thread = await message.startThread({
    name: `${embed.data.title} • ${new Intl.DateTimeFormat("en-US", { month: "short", day: "numeric" }).format(timestamp)}`,
...
export const detectBeaconIntent = (msg) =>
  [
    /anyone want to (run|hike|bike|cycle)/i,
    /looking for (a|someone to) (run|hike|bike|cycle)/i,
    /who'?s up for (a )?(run|hike|bike|cycle)/i,
  ].some((r) => r.test(msg.content));

export const sendBeaconTip = (msg) =>
  msg.reply({
    embeds: [
      new EmbedBuilder()
        .setColor("#5865F2")
        .setDescription(
          "💡 **Tip**: Use `/beacon [details]` to create a meetup beacon!\nExample: `/beacon tomorrow 9am run at riverside`",
        ),
    ],
  });
...
export const testcronCommand = {
  ...
  async execute(interaction) {
    ...
    return interaction.editReply({
      content: `🧹 Background cleanup completed! Swept ${cleanedCount} past events.`,
    });
  },
};
...
export const preferencesCommand = {
  ...
  async execute(interaction) {
    ...
    const rows = roles.map((r) =>
      new ActionRowBuilder().addComponents(
        new StringSelectMenuBuilder()
          ...
          .addOptions(
            PREFERENCE_CONFIGS[roleMap[r]].map((c) => ({
              label: c.label,
              value: c.id,
            })),
          ),
    ...
    const embed = new EmbedBuilder()
      .setColor("#2B2D31")
      .setTitle("Personalize Your Preferences")
      .setDescription(
